...
class DatetimeFilters extends FilterCollection {
    ...
    static function relative_time($timestamp, $format = 'g:iA') {
        ...
        if ($delta > 86400 && $timestamp < $time) {
            return "Gestern um " . date("g:i a", $timestamp);
        }

        if ($delta > 7200)
            $string .= floor($delta / 3600) . " Stunden, ";
        else if ($delta > 3660)
            $string .= "1 Stunde, ";
        else if ($delta >= 3600)
            $string .= "1 Stunde ";
        ...
        if ($delta > 60)
            $string .= floor($delta / 60) . " Minuten ";
        else
            $string .= $delta . " Sekunden ";
        return "vor $string";
    }
    ...
    static function relative_date($time) {
        ...
        if (abs($reldays) < 7) {
            if ($reldays > 0) {
                ...
                return 'in ' . $reldays . ' Tag' . ($reldays != 1 ? 'en' : '');
            } else {
                $reldays = abs(floor($reldays));
                return 'vor '.$reldays . ' Tag'  . ($reldays != 1 ? 'en' : '');
            }
        }
        if (abs($reldays) < 182)
